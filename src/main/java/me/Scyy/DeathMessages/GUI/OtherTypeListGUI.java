...
import me.Scyy.DeathMessages.GUI.Prompts.OtherCauseMessagePrompt;
...
public class OtherTypeListGUI extends InventoryGUI {
    ...
    public OtherTypeListGUI(InventoryGUI lastGUI, Plugin plugin, Player player, UUID target, int page) {
        ...
        this.page = page;
        this.target = target;
        List<ItemStack> items = plugin.getConfigManager().getSettings().getOtherDeathItems();
    ...
    public InventoryGUI handleClick(InventoryClickEvent event) {
        ...
        if (listSlot != -1 && inventoryItems[click] != null) {
            ...
            Conversation conv = factory.withFirstPrompt(new OtherCauseMessagePrompt(plugin, selected)).withLocalEcho(false)
    ...
    private DamageCause getType(ItemStack item) {
        ...
        String rawCause = lore.get(lore.size() - 1).split(": ")[1];
        try {
            return DamageCause.valueOf(rawCause);
